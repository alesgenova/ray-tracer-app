...
impl Actor {
    pub fn new(hitable: Hitable, material: Material) -> Self {
        Actor {
            wrapped: Box::new(ActorInternal{ hitable: hitable.wrapped, material: material.wrapped })
        }
    }
}
...
impl Renderer {
    pub fn new(width: usize, height: usize, sampling: usize, reflections: usize, antialiasing: bool) -> Self {
        Renderer {
            width,
            height,
            wrapped: Box::new(RendererInternal::new(width, height, sampling, reflections, antialiasing))
        }
    }
    ...
    pub fn render_pixel(&self, i: usize, j: usize, scene: &Scene, camera: &Camera, color: &mut [f64]) {
        let inner_color = self.wrapped.render_pixel(i, j, &scene.wrapped, &*camera.wrapped);
        for i in 0..3 {
