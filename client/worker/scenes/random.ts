...
export default function (aspect: number, seed: number) {
  let rng = makeRng(seed);

  let scene = Scene.new();
  // scene.set_background([0.5, 0.7, 0.9]);
  scene.set_background([0.25, 0.35, 0.45] as any);
  ...
  for (let i = 0; i < N_SPHERES_X; ++i) {
    for (let j = 0; j < N_SPHERES_Y; ++j) {
      let radius = MIN_RADIUS + (MAX_RADIUS - MIN_RADIUS) * rng();
      let x = i + rng() * (1.0 - radius);
      x = MIN_X + (MAX_X - MIN_X) * x / N_SPHERES_X;
      let y = j + rng() * (1.0 - radius);
      ...
      if (hitableRand < SPHERE_PROBABILITY) {
        ...
        hitable = Hitable.translation(hitable, [x, y, radius] as any);
      } else {
        let l = radius * 2.0 * 0.75;
        hitable = Hitable.cube(l, l, l);
        hitable = Hitable.translation(hitable, [x, y, radius * 0.75] as any);
      }

      let color = [rng(), rng(), rng()];
      let texture = Texture.uniform(color as any);

      let materialRand = rng();
      let material;
      if (materialRand < LAMBERTIAN_PROBABILITY) {
        material = Material.lambertian(texture, 0.65);
      } else if (materialRand < METAL_PROBABILITY) {
        let fuzziness = MIN_FUZZINESS + (MAX_FUZZINESS - MIN_FUZZINESS) * rng();
        material = Material.metal(texture, fuzziness);
      } else {
        let n = MIN_REFRACTIVE + (MAX_REFRACTIVE - MIN_REFRACTIVE) * rng();
  ...
  let radius = 2.0;
  {
    ...
    hitable = Hitable.translation(hitable, [0.0, 0.0, radius] as any);
  ...
  {
    let hitable = Hitable.sphere(radius);
    hitable = Hitable.translation(hitable, [0.0, - 2.1 * radius, radius] as any);
  ...
  // Floor
  {
    ...
    let texture1 = Texture.uniform([0.85, 0.85, 0.85] as any);
  ...
  // Light
  {
    ...
    hitable = Hitable.translation(hitable, [0.0, 5.0, 15.2] as any);
